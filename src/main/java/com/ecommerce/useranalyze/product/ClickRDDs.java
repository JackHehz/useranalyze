 ...
 *查询指定日期范围内的点击行为数据
 */
public class ClickRDDs {

	public static JavaPairRDD<Long, Row> getClickActionRDDByDate(
			SQLContext sqlContext, String startDate, String endDate) {
		// 从user_visit_action中，查询用户访问行为数据
		// 1.：click_product_id，限定为不为空的访问行为，那么就代表着点击行为
		// 2.：在用户指定的日期范围内的数据
	...
	public static JavaPairRDD<Long, Row> getcityid2CityInfoRDD(SQLContext sqlContext) {
		...
		if(local) {
			...
			user = ConfigurationManager.getProperty(Constants.JDBC_URL_PROD);
			password = ConfigurationManager.getProperty(Constants.JDBC_PASSWORD_PROD);
		}
	 ...
	 * @param sqlContext
	 */
	public static void generateTempAreaPrdocutClickCountTable(
			SQLContext sqlContext) {
...
//				+ "JOIN tmp_product_info pi ON tapcc.product_id=pi.product_id ";
	 ...
	 * @return
	 */
	public static JavaRDD<Row> getAreaTop5ProductRDD(SQLContext sqlContext) {
		
		// 使用开窗函数先进行一个子查询
		// 按照area进行分组，给每个分组内的数据，按照点击次数降序排序，打上一个组内的行号
		// 接着在外层查询中，过滤出各个组内的行号排名前5的数据
	 ...
	 * @param rows
	 */
	public static void persistAreaTop5Product(long taskid, List<Row> rows) {
		List<AreaTop5Product> areaTop5Products = new ArrayList<AreaTop5Product>();
		
		for(Row row : rows) {
			AreaTop5Product areaTop5Product = new AreaTop5Product();
			areaTop5Product.setTaskid(taskid); 
			areaTop5Product.setArea(row.getString(0));  
			areaTop5Product.setAreaLevel(row.getString(1));  
			areaTop5Product.setProductid(row.getLong(2)); 
			areaTop5Product.setClickCount(Long.valueOf(String.valueOf(row.get(3))));    
			areaTop5Product.setCityInfos(row.getString(4));  
			areaTop5Product.setProductName(row.getString(5));  
			areaTop5Product.setProductStatus(row.getString(6));  
			areaTop5Products.add(areaTop5Product);
		}
		
		AreaTop5ProductDAO areTop5ProductDAO = DAOFactory.getTop5ProductDAO();
		areTop5ProductDAO.insertBatch(areaTop5Products);
	}
}
